...
//			if(pgsStCdVal == "Z" || pgsStCdVal == "3" || pgsStCdVal == "A"){
	if(pgsStCdVal == "Z"){
		if(Number(sessAuth) > 4 || sessMstu == "M"){
...
return yyyy+mm+dd;
	 ...
	 */
	 function getCommonCode(codkndStr, gridYn, completeFc, url){
	 	url = typeof(url) == "undefined" ? "/common/getCommonCombo.do" : url.substring(0,1) == "/" ? ctxPath+url : url;
	 	$.ajax( {
	 		type :'POST'
	 		,asyn :true
	 		,url :url
...
,complete : function(xhr, textStatus) {
	 ...
	 * @returns null
	 */
	 function doCommonFormAjax(url, paramForm, successFc){
...
$("#pop_yes").click(function(){
	$.unblockUI({
		onUnblock: function(){
			...
			doCommonAjax("/login/logOut.do", paramData, "top.location.href = '/index.jsp'");
		}
	});
});
}
...
function myApprovalPopup(){
	...
	var url = "/approval/approvalPopup.do";
	var width = "650";
	var height = "530";
	...
	$("#approvalForm input[name=hid_csrfToken]").val($("#csrfToken").val());
	$("#approvalForm").attr("target", "POP_MY_APPR");
	$("#approvalForm").attr("method", "POST");
	$("#approvalForm").attr("action", url).submit();
...
form.attr("action" , "/system/menuManagement.do");
form.attr("target" , "");
var token = $("<input type='hidden' id='hid_csrfToken' name='hid_csrfToken'/>").val($("#csrfToken").val());
form.append(token);
$("body").append(form);
...
//              pageNavigator += "<li style=\"cursor:pointer;\" onclick=\"fnPageMove('" + (start_page_num - pageNumSize) + "');\"><img src=\""+ctxPath+"/images/common/page/prev.gif\"/></li>";                     // 이전 페이지
                pageNavigator += "<a class=\"pagenum_item\" href=\"javascript:fnPageMove('1');\" class=\"btn_first\"><span>처음</span></a>";             // 첫 페이지
                pageNavigator += "<a class=\"pagenum_item\" href=\"javascript:fnPageMove('" + (start_page_num - pageNumSize) + "');\" class=\"btn_prev\"><span>이전</span></a>";             // 이전 페이지
              }
              for (var i = start_page_num; i <= end_page_num; i++) {
              	if (nowPage == i) {
//                  pageNavigator += "<li class=\"current\">"+i+"</li>";
                    pageNavigator += "<a class=\"pagenum_item pagactive\" href=\"javascript:;\">" + i + "</a>";     // 현재 페이지
                  } else {
//                  pageNavigator += "<li style=\"cursor:pointer;\" onclick=\"fnPageMove('" + i + "');\">"+i+"</li>";
                    pageNavigator += "<a class=\"pagenum_item\" href=\"javascript:fnPageMove('" + i + "');\">" + i + "</a>";           // 다른 페이지
                  }
                }

            if (total_page_cnt > end_page_num) {                                                                            // 마지막 페이지가 전체 페이지보다 작을 경우
                pageNavigator += "<a class=\"pagenum_item\" href=\"javascript:fnPageMove('" + next_end_page_num + "');\" class=\"btn_next\"><span>다음</span></a>";              // 다음페이지
                pageNavigator += "<a class=\"pagenum_item\" href=\"javascript:fnPageMove('" + total_page_cnt + "');\" class=\"btn_last\"><span>마지막</span></a>";                // 마지막 페이지
...
return ""+yy+getMessage("COMM.0010")+" "+mm+getMessage("COMM.0011")+" "+dd+getMessage("COMM.0012");
	...
	function confirmUI2(txt, gbWidth, gbHeight){
		var popTop = ($(window).height() - Number(gbHeight)) /2 + 'px';
		var popLeft = ($(window).width() - Number(gbWidth)) /2 + 'px';
		var css = {
			top: popTop,
...
$(".tab2_bt").click(function(){
	...
	/* HTML TABLE FUNCTION START */
	var fnHtmlTable = {
	tableNames : new Array(), // table id array
	tableBodys : new Array(), // table tr tag array
	tableNavis : new Array(), // table navigator array
	tableUrls  : new Array(), // table action url array
	tableMulti : new Array(), // table multi combo json array
	dtlParams  : {},
	/*
	 * tableInit : 화면 내 Table 초기화
	 * tNames         : Array<String> : 테이블 ID 배열
	 * tUrls          : Array<Json>   : 테이블ID별 Server 요청 Url
	 * collBackFc     : String        : Init 후 실행할 CollBack 함수
	 * initSearchFlag : boolean       : 테이블 Init 후 Search 여부
	 */
	 tableInit : function (tNames, tUrls, collBackFc, initSearchFlag, tMulti, addParams){
		var tBodys = new Array(); // table tr tag Array var
		var tNavis = new Array(); // table navigator Array var

		try{
			for(var i=0; i<tNames.length; i++){
				if($("#"+tNames[i]) == 'undefined'){ // table id check
					alert("Wrong " + (i + 1) + " Index Table ID"); // not exist used table id
					return;
				}

				if(!fnHtmlTable.getUndefinedFlag(tMulti) && tMulti.length > 0){ // 멀티콤보용 JSON Data가 있는지 여부
					if(tMulti[i] != ""){
						$.each(tMulti[i], function(idx, mJson){ // 있을경우 들어가있는 ID별로 for문
							$.each(mJson, function(i, data){ // ID별 JSON Data를 가져온다.
								$("#" + idx).append( $("<option>").text(data.name).attr("value", data.value)); // option 추가
							});
						});
					}
				}

				tBodys[i] = $("#"+tNames[i]).html(); // copy table body's tag
				tNavis[i] = tNames[i] + "_navigator"; // table navigator

				$("#"+tNames[i]).empty(); // table empty

				// 추후에 상세로 넘어가는 부분 수정시 해당 function 사용
				$("#"+tNames[i]).on("click", ".go_detail", function(){ // td class중에 go_detail을 가지고 있을 경우 상세보기로 가는 이벤트 추가
					fnHtmlTable.doTableDetail( // 상세페이지로 이동
							  fnHtmlTable.getTableIndexById($(this).closest("table").attr("id")) // 테이블 ID로 테이블 Index 가져오기
							, {}, {}, tNames // 자동으로 호출 하는 여기에서는 별도 파라메터를 넣어주지 않는다. :: 업무단에서 직접 호출 시 넣어줌
							);
				});

				// MultiCombo 이벤트
				$("#"+tNames[i]).on("change", ".multiple", fnHtmlTable.setTableMultiCombo);

				// 전체체크 이벤트
				if(!fnHtmlTable.getUndefinedFlag($("#"+tNames[i]+"_all_chk"))){
					$(document).on("click", "#"+tNames[i]+"_all_chk", function(){
						var tblId = $(event.target).attr("id").replace("_all_chk", "");
						$("#"+tblId).find("[id^='chk']").attr("checked", true);
					});
				}

				// 검색조건 enter키 입력시 검색
				$("#" + tNames[i]).on("keyup", ".", function(){
					if(window.event.keyCode == 13){
						if(!fnHtmlTable.getUndefinedFlag(collBackFc)){
							fnHtmlTable.doTableSearch(0, initPage, collBackFc);
						}else{
							fnHtmlTable.doTableSearch(0, initPage);
						}
					}
				});

				// 목록에서 입력받는 경우 선택된Row와 선택하지 않은 Row의 CSS 차별 두기
				if(!fnHtmlTable.getUndefinedFlag($("#"+tNames[i]).find(".readonly"))){
					$("#"+tNames[i]).on("focus", ".input_modify", function(){
						$(this).closest("tr").removeClass("readonly");
					});
					$("#"+tNames[i]).on("blur", ".input_modify", function(){
						$(this).closest("tr").addClass("readonly");
					});
				}
				...
				if(!fnHtmlTable.getUndefinedFlag($("#"+tNames[i]).find(".readonly"))){
					$("#"+tNames[i]).on("focus", ".input_modify", function(){
						$(this).closest("ul").removeClass("readonly");
					});
					$("#"+tNames[i]).on("blur", ".input_modify", function(){
						$(this).closest("ul").addClass("readonly");
					});
				}

				$("."+tNames[i] + "_scroll").mCustomScrollbar({
					axis:"y",
					theme:"dark",
			...
			fnHtmlTable.tableNames = tNames; // copy array
			fnHtmlTable.tableBodys = tBodys; // copy array
			fnHtmlTable.tableNavis = tNavis; // copy array
			fnHtmlTable.tableUrls  = tUrls;  // copy array
			if(!fnHtmlTable.getUndefinedFlag(tMulti)){
				fnHtmlTable.tableMulti = tMulti;
			}
			if(initSearchFlag){ // 기본 조회
				var initTable = 0;
				var initPage = 1;

				if( !fnHtmlTable.getUndefinedFlag(addParams) &&
					!fnHtmlTable.getUndefinedFlag(addParams.initPage)){
					initPage = addParams.initPage;
			}

			if(!fnHtmlTable.getUndefinedFlag(collBackFc)){
				fnHtmlTable.doTableSearch(initTable, initPage, collBackFc);
			}else{
				fnHtmlTable.doTableSearch(initTable, initPage);
			}
			}else if(!fnHtmlTable.getUndefinedFlag(collBackFc)){ // execution call back function
				eval(collBackFc);
			}
		}catch(e){ // exception
			console.log(e);
			alert("Wrong Table Init Parameter!");
		}
	},
	tableInitAddRows : function(addCount, collBackFc){ // All table add rows
		for(var i=0; i<fnHtmlTable.tableNames.length; i++){
			for(var j=0; j<addCount; j++){
				$("#" + fnHtmlTable.tableNames[i]).append(fnHtmlTable.tableBodys[i].replace(/_0/g, "_" + fnHtmlTable.getTableMaxRowNum(i)));
			}
		}

		if(typeof(collBackFc) != 'undefined'){ // execution call back function
			eval(collBackFc);
		}
	},
	tableAddRows : function(tableIndex, addCount, collBackFc){ // one table add rows
		var count = 0;
		if(typeof(addCount) != 'undefined'){
			count = addCount;
		}
		for(var i=0; i<count; i++){
			$("#" + fnHtmlTable.tableNames[tableIndex]).append(fnHtmlTable.tableBodys[tableIndex].replace(/_0/g, "_" + fnHtmlTable.getTableMaxRowNum(tableIndex)));
		}
	...
	tableDelRows : function(tableIndex, collBackFc){
		var chkBoxArr = $("#"+fnHtmlTable.getTableIdByIndex(tableIndex)).find("[id^='chk_']:checked");

		for(var i=0; i<chkBoxArr.length; i++){
			$(chkBoxArr[i]).closest("ul").remove();
		}
		if(typeof(collBackFc) != 'undefined'){ // execution call back function
			eval(collBackFc);
		}
	},
	tableDelTrRows : function(tableIndex, collBackFc){
		var chkBoxArr = $("#"+fnHtmlTable.getTableIdByIndex(tableIndex)).find("[id^='chk_']:checked");

		for(var i=0; i<chkBoxArr.length; i++){
			$(chkBoxArr[i]).closest("tr").remove();
		}
		if(typeof(collBackFc) != 'undefined'){ // execution call back function
			eval(collBackFc);
		}
	},
	getTableIndexById : function(tableId){
		var tIdx = 0;

		for(var i=0; i<fnHtmlTable.tableNames.length; i++){
			if(fnHtmlTable.tableNames[i] == tableId){
				tIdx = i;
				break;
			}
		}
	...
	getTableIdByIndex : function(tableIndex){
		return fnHtmlTable.tableNames[tableIndex];
	},
	setTableMultiCombo : function(tObj){

		if($(tObj)[0].tagName != "SELECT"){
			tObj = $(event.target);
		}

		// 변경할 JSONDATA List를 가져오기 위한 Table Index
		var tableIdx = fnHtmlTable.getTableIndexById($(tObj).closest("table").attr("id"));
		// 상위 콤보 ID
		var parentId = $(tObj).attr("id");

		if($(tObj).attr("child") == null){
			return;
		}
		// 하위 콤보 ID (구분자 , 로 여러개 가능)
		var childArr = $(tObj).attr("child").split(",");

		// 하위콤보만큼 반복
		for(var i=0; i<childArr.length; i++){
			// 처음 시작일 경우에는 상위콤보 ID가 셋팅되어 있어서 별도로 셋팅하지 않음
			if(parentId != "" && i > 0){
				// 하위콤보가 여러개일 경우 부모콤보ID를 가져온다.
				parentId = childArr[i-1];
			}

			// 변경할 하위콤보 ID 가져옴
			var childId  = $("#" + childArr[i]).attr("id");
			// 변경할 하위콤보 JSONDATA List를 가져온다.
			var multiData = fnHtmlTable.tableMulti[tableIdx][childId.substring(0, childId.lastIndexOf("_") + 1) + "0"];
			...
			$("#" + childId).append( $("<option>").text("선택").attr("value", ""));
			// JSONDATA 기반으로 콤보의 내용을 변경한다.
			for(var comIdx=0; comIdx<multiData.length; comIdx++){
				// 상위콤보값과 같은것만 추가
				if(multiData[comIdx].key == $("#" + parentId).val()){
					$("#" + childId).append( $("<option>").text(multiData[comIdx].name).attr("value", multiData[comIdx].value)); // option 추가
				}
			}
			$("#" + childId).val(childVal);
		}
	},
	setTableClear : function(tableIndex, collBackFc){ // one table clear
		if(typeof($("#" + fnHtmlTable.tableNames[tableIndex])) == 'undefined'){
			for(var i=0; i<fnHtmlTable.tableNames.length; i++){
				if($("#"+fnHtmlTable.tableNames[i]) == 'undefined'){
					alert("Wrong " + (i + 1) + " Index Table ID");
					return;
				}

				$("#"+fnHtmlTable.tableNames[i]).empty();
			}
		}else{
			$("#" + fnHtmlTable.tableNames[tableIndex]).empty();
		}
	...
	getTableMaxRowNum : function(tableIndex){ // one table max rownum
		var trs = fnHtmlTable.getTableTrsArray(tableIndex);
		var maxRowNum = 0;
		var tmpRowNum = 0;

		for(var i=0; i<trs.length; i++){
			tmpRowNum = fnHtmlTable.getTableTrNumByTr(trs[i]);
			if( parseInt(maxRowNum, 10) < parseInt(tmpRowNum, 10) ||
				parseInt(maxRowNum, 10) == parseInt(tmpRowNum, 10) ){
				maxRowNum = parseInt(tmpRowNum, 10) + 1;
		}
	}
	...
	getTableLisArray : function(tableIndex){ // get one table all tr
		var trs;

		try{
			trs = $("#" + fnHtmlTable.tableNames[tableIndex] + ">li");
		}catch(e){
			alert("Except Get TR Array!");
		}
		return trs;
	},
	getTableUlsArray : function(tableIndex){ // get one table all tr
		var trs;

		try{
			trs = $("#" + fnHtmlTable.tableNames[tableIndex] + ">ul");
		}catch(e){
			alert("Except Get TR Array!");
		}
		return trs;
	},
	getTableTrsArray : function(tableIndex){ // get one table all tr
		var trs;
		try{
			trs = $("#" + fnHtmlTable.tableNames[tableIndex] + ">tr");
			if(trs.length == 0){
				trs = $("#" + fnHtmlTable.tableNames[tableIndex] + ">ul");
			}
		}catch(e){
	...
	getTableTrNumByTr : function(tr){
		var trNum = "";

		try{
			trNum = $(tr).attr("id").replace("trNum_", "");
			parseInt(trNum);
			trNum = trNum.toString();
		}catch(e){
			alert("TR Tag Attribute ID is not initialized");
		}

		return trNum;
	},
	getTableTrNumByChild : function(child){
		var trNum = "";

		try{
			trNum = $(child).closest("li").attr("id").replace("trNum_", "");
			parseInt(trNum);
			trNum = trNum.toString();
		}catch(e){
			alert("TR Tag Attribute ID is not initialized");
	...
	getTableChkRow : function(tableIndex){
		var chkBoxArr = $("#" + fnHtmlTable.getTableIdByIndex(tableIndex)).find("[id^=chk]:checked");
		var idArr = [];
		for(var i=0; i<chkBoxArr.length; i++){
			idArr.push($(chkBoxArr[i]).attr("id").replace("chk_", ""));
		}
		return idArr;
	},
	getColValue : function(colName, row){
		var tagName = $("#" + colName + "_" + row)[0].tagName.toLowerCase();
		var colVal  = "";

		switch(tagName){
			case "select" :
			colVal = $("#" + colName + "_" + row).val();
			break;
			case "input" :
			switch($("#" + colName + "_" + row).prop("type").toLowerCase()){
				case "checkbox" :
				colVal = $("#" + colName + "_" + row).is(":checked") ? "Y" : "N";
				break;
				case "radio" :
				colVal = $("[name='"+colName+"']:checked").val();
				break;
				default :
				colVal = $("#" + colName + "_" + row).val();
			}
...
getSearchOption : function(tableIndex, getType){
	var tableName = fnHtmlTable.tableNames[tableIndex];
	var tabId = "tab";
	if(tableName.indexOf("popUp") == 0){
		tabId = "popUpTab";
	}
	var searchField = $("#" + tabId + (tableIndex + 1)).find(".");
	var optionStr = "";
	var params = {};

	for(var i=0; i<searchField.length; i++){
		optionStr += $(searchField[i]).val() + "|";

		if($(searchField[i]).hasClass("hasDatepicker")){
			params[$(searchField[i]).attr("id")] = selectNum($(searchField[i]).val());
		}else{
			params[$(searchField[i]).attr("id")] = $(searchField[i]).val();
		}
	}

	params["hid_cond"]    = optionStr;
	params["hid_pagenum"] = $("#"+ fnHtmlTable.tableNavis[tableIndex]).find(".active").text();

	if(typeof(getType) != 'undefined'){
		switch(getType){
			case "DTL" :
			params["hid_view_nm"]   = $("#menu_id").val();
			params["hid_page"]      = $("#hid_page").val();
			params["hid_csrfToken"] = $("#csrfToken").val();
			break;
		}
	}
...
doTableSearch : function(tableIndex, nPage, successFc, addParam){
	var url = fnHtmlTable.tableUrls[tableIndex].search;
	url = url.substring(0,1) == "/" ? ctxPath+url : url;

	if( $("#"+fnHtmlTable.tableNavis[tableIndex]) != 'undefined' &&
		nPage == 'undefined'){
		nPage = 1;
}
		// common global json data
		var corpCd = sessCorpCd;
		var locale = sessLocale;

		var params = fnHtmlTable.getSearchOption(tableIndex);
		for(add in addParam){
			params[add] = addParam[add];
		}
		//
		...
		$.ajax( {
			...
			,success : function(jsonData) {
				...
				if( $("#"+fnHtmlTable.tableNavis[tableIndex]) != 'undefined'){
					$("#"+fnHtmlTable.tableNavis[tableIndex]).html(fnHtmlTable.getPageNavigator(nPage, jsonData.records, tableIndex));
				}

				if(!fnHtmlTable.getUndefinedFlag($("#mobile_total")[0])){
					$("#mobile_total").text(jsonData.records + getMessage("thing"));
				}

				$("#"+fnHtmlTable.tableNames[tableIndex]).empty();
				if( typeof($(fnHtmlTable.tableNames[tableIndex])) != 'undefined'){
					fnHtmlTable.setTableRowDatas(
						fnHtmlTable.tableNames[tableIndex]
						, fnHtmlTable.tableBodys[tableIndex]
						, jsonData.rows);
				}

				var multiComboArr = $("#"+fnHtmlTable.tableNames[tableIndex]).find(".multi_first");

				if(multiComboArr.length > 0){
					for(var i=0; i<multiComboArr.length; i++){
                		//alert(1)
                		fnHtmlTable.setTableMultiCombo(multiComboArr[i]);
                	}
                }
	...
	doTableDetail : function(tableIndex, paramData, successFc, tNames){
		/*var readonly   = $("#"+tNames).find(".readonly");
		var litr   	   = readonly.prop('tagName').toLowerCase();
		var tr		   = "";
		switch(litr) {
		case "tr" :
		...
		var tr		 = "";
		var readonly   = $("#"+tNames).prop('tagName').toLowerCase();
		switch(readonly) {
			case "ul" :
			tr     = $(event.target).closest("li");
			break;
...
var rowId  = selectNum($(tr).attr("id"));
var tblIdx = 0;

if(tableIndex == ""){
	tblIdx = fnHtmlTable.getTableIndexById($(event.target).closest("table").attr("id"));
}else{
	tblIdx = tableIndex;
}

var keys = $(tr).find(".key");
var dtlParam = fnHtmlTable.getSearchOption(tblIdx, "DTL");

var paramKey = "";
var paramVal = "";
var url      = "";

if( fnHtmlTable.getUndefinedFlag($("#url_type_" + rowId)[0]) ){
	url = fnHtmlTable.tableUrls[tblIdx].detail;
}else{
	url = fnHtmlTable.tableUrls[tblIdx]["detail_"+fnHtmlTable.getColValue("url_type", rowId)];
}

for(var j=0; j<keys.length; j++){
	paramKey = "hid_" + $(keys[j]).attr("id").replace("_"+rowId, "");

	switch($(keys[j]).prop("tagName").toLowerCase()){
		case "select" :
		case "input"  :
		paramVal = $.trim($(keys[j]).val());
		break;
		default :
		paramVal = $.trim($(keys[j]).text());
	}

	dtlParam[paramKey] = paramVal;
}
fnHtmlTable.doFormSubmit(url, dtlParam);
},
doFormSubmit : function(url, param, Id){
	var formId = "hideForm";
	...
	var form = $("<form method='post' id='"+formId+"' action='"+url+"'/>");
	for(obj in param){
		var hid = $("<input type='hidden' id='"+obj+"' name='"+obj+"'/>").val(param[obj]);
		form.append(hid);
	}
...
setTableRowDatas : function(tableName, trTag, rows, successFc){
	var tdnm =  $("#"+tableName);
	$.each(rows, function(iRow, datas){
		$("#"+tableName).append(trTag.replace(/_0/g,"_"+(iRow+1)));
		$.each(datas, function(idx, opt){
			if( $("#"+tableName+" #"+idx+"_"+(iRow+1))[0] != null){
				switch($("#"+tableName+" #"+idx+"_"+(iRow+1)).prop("tagName").toLowerCase()){
					case "font" :
					case "td" :
					case "a" :
					case "span" :
					case "p" :
					if($("#"+tableName+" #"+idx+"_"+(iRow+1)).hasClass("amt")){
						$("#"+tableName+" #"+idx+"_"+(iRow+1)).html(numberComma(opt));
					}else{
						$("#"+tableName+" #"+idx+"_"+(iRow+1)).html(opt);
					}
					break;
					case "textarea" :
					case "select" :
					$("#"+tableName+" #"+idx+"_"+(iRow+1)).on("change keypress", function() {
                             //select타입 변경시 첫번재 td의 체크박스 체크
                             $(this).parent().parent().find('td:eq(0) input[type=checkbox]').prop("checked",true);
                           });

					$("#"+tableName+" #"+idx+"_"+(iRow+1)).val(opt);
					break;
					case "img" :
					if(opt == ""){
//                            $("#"+idx+"_"+(iRow+1)).attr("src", ctxPath+"/images/common/no_image.png");
$("#"+tableName+" #"+idx+"_"+(iRow+1)).hide();
}else{
	if(idx.indexOf('file') > -1){
		$("#"+tableName+" #"+idx+"_"+(iRow+1)).attr("src", ctxPath+"/images/btn/btn_filedown_tbl.png");
	} else {
		$("#"+tableName+" #"+idx+"_"+(iRow+1)).attr("src", ctxPath+"/upload"+opt);
	}
}
break;
case "input" :
switch($("#"+tableName+" #"+idx+"_"+(iRow+1)).prop("type").toLowerCase()){
	case "checkbox" :
	$("#"+tableName+" #"+idx+"_"+(iRow+1)).prop("checked", opt == "Y" ? true : false);
	break;
	default :
	if($("#"+tableName+" #"+idx+"_"+(iRow+1)).hasClass("amt")){
		$("#"+tableName+" #"+idx+"_"+(iRow+1)).val(numberComma(opt));
	}else{
		$("#"+tableName+" #"+idx+"_"+(iRow+1)).val(opt);
	}
}
...
getPageNavigator : function(nowPage, totalCnt, tableIndex){
	var pageNavigator = "";
	if(totalCnt == 0){
            pageNavigator = "<span><font style='font-size: 16px;'>"+getMessage("SEARCH.0002")+"</font></span>";//조회된 데이터가 없습니다.
          }else{
            ...
            if (start_page_num > 1) {                                                                                       // 현재 페이지가 첫 페이지가 아닐 경우
                pageNavigator += "<a class=\"first\" href=\"javascript:fnHtmlTable.pageMove('1', " + tableIndex + ");\"><span class=\"icon\"></span></a>";             // 첫 페이지
                pageNavigator += "<a class=\"prev\" href=\"javascript:fnHtmlTable.pageMove('" + (start_page_num - pageNumSize) + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";             // 이전 페이지
              }
              for (var i = start_page_num; i <= end_page_num; i++) {
              	if (nowPage == i) {
                    pageNavigator += "<a class=\"active\" href=\"javascript:;\">" + i + "</a>";     // 현재 페이지
                  } else {
                    pageNavigator += "<a href=\"javascript:fnHtmlTable.pageMove('" + i + "', " + tableIndex + ");\">" + i + "</a>";           // 다른 페이지
                  }
                }
            if (total_page_cnt > end_page_num) {                                                                            // 마지막 페이지가 전체 페이지보다 작을 경우
                pageNavigator += "<a class=\"next\" href=\"javascript:fnHtmlTable.pageMove('" + next_end_page_num + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";              // 다음페이지
                pageNavigator += "<a class=\"last\" href=\"javascript:fnHtmlTable.pageMove('" + total_page_cnt + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";                // 마지막 페이지
              }
            }

            return pageNavigator;
          },
          pageMove : function(page, tIndex){
          	var moveCallBack = "";
          	if(fnHtmlTable.getUndefinedFlag(fnHtmlTable.tableUrls[tIndex].search_callback)){
          		fnHtmlTable.doTableSearch(tIndex, page);
          	}else{
          		fnHtmlTable.doTableSearch(tIndex, page, fnHtmlTable.tableUrls[tIndex].search_callback);
          	}
          },
          getUndefinedFlag : function(object){
          	return typeof(object) == 'undefined';
          },
          setTableReset : function(collBackFc){
          	for(var i=0; i<fnHtmlTable.tableNames.length; i++){
          		fnHtmlTable.setTableClear(i);
          		$("#" + fnHtmlTable.tableNames[i]).append(fnHtmlTable.tableBodys[i]);
          	}
          ...
          showNewPop : function(targetId, width, height, param){
          	if(fnHtmlTable.getUndefinedFlag(param)){
          		$("#" + targetId).find("input").val("");
          		var selectBox = $("#" + targetId).find("select");
          		for(var i=0; i<selectBox.length; i++){
          			selectBox[i].selectedIndex = "0";
          		}
          	}else{
          		for(popData in param){
          			$("#" + popData).val(param[popData]);
          		}
          	}
          ...
          fnFormReset : function(formId){
          	$("#mobile_search_form")[0].reset();
          }
        }
        /* HTML TABLE FUNCTION END */

        var fnPopUpHtmlTable = {
		tableNames : new Array(), // table id array
		tableBodys : new Array(), // table tr tag array
		tableNavis : new Array(), // table navigator array
		tableUrls  : new Array(), // table action url array
		 ...
		 tableInit : function (tNames, tUrls, collBackFc, initSearchFlag, tMulti, addParams){
			...
			try{
				for(var i=0; i<tNames.length; i++){
					...
					if(!fnPopUpHtmlTable.getUndefinedFlag(tMulti) && tMulti.length > 0){ // 멀티콤보용 JSON Data가 있는지 여부
						$.each(tMulti[i], function(idx, mJson){ // 있을경우 들어가있는 ID별로 for문
							$.each(mJson, function(i, data){ // ID별 JSON Data를 가져온다.
								$("#" + idx).append( $("<option>").text(data.name).attr("value", data.value)); // option 추가
							});
					...
					// 추후에 상세로 넘어가는 부분 수정시 해당 function 사용
					$("#"+tNames[i]).on("click", ".go_detail", function(){ // td class중에 go_detail을 가지고 있을 경우 상세보기로 가는 이벤트 추가
						fnPopUpHtmlTable.doTableDetail( // 상세페이지로 이동
								  fnPopUpHtmlTable.getTableIndexById($(this).closest("table").attr("id")) // 테이블 ID로 테이블 Index 가져오기
								, {} // 자동으로 호출 하는 여기에서는 별도 파라메터를 넣어주지 않는다. :: 업무단에서 직접 호출 시 넣어줌
								);
					});

					// MultiCombo 이벤트
					$("#"+tNames[i]).on("change", ".multiple", fnPopUpHtmlTable.setTableMultiCombo);

					// 전체체크 이벤트
					if(!fnPopUpHtmlTable.getUndefinedFlag($("#"+tNames[i]+"_all_chk"))){
						$(document).on("click", "#"+tNames[i]+"_all_chk", function(){
							var tblId = $(event.target).attr("id").replace("_all_chk", "");
							$("#"+tblId).find("[id^='chk']").attr("checked", true);
						});
					...
					$("#" + tNames[i]).on("keyup", ".", function(){
						if(window.event.keyCode == 13){
							if(!fnPopUpHtmlTable.getUndefinedFlag(collBackFc)){
								fnPopUpHtmlTable.doTableSearch(0, initPage, collBackFc);
							}else{
								fnPopUpHtmlTable.doTableSearch(0, initPage);
							}
						}
					});

					// 목록에서 입력받는 경우 선택된Row와 선택하지 않은 Row의 CSS 차별 두기
					if(!fnPopUpHtmlTable.getUndefinedFlag($("#"+tNames[i]).find(".readonly"))){
						$("#"+tNames[i]).on("focus", ".input_modify", function(){
							$(this).closest("tr").removeClass("readonly");
						});
						$("#"+tNames[i]).on("blur", ".input_modify", function(){
				...
				fnPopUpHtmlTable.tableNames = tNames; // copy array
				fnPopUpHtmlTable.tableBodys = tBodys; // copy array
				fnPopUpHtmlTable.tableNavis = tNavis; // copy array
				fnPopUpHtmlTable.tableUrls  = tUrls;  // copy array
				if(!fnPopUpHtmlTable.getUndefinedFlag(tMulti)){
					fnPopUpHtmlTable.tableMulti = tMulti;
				}
				...
				if(initSearchFlag){ // 기본 조회
					...
					if( !fnPopUpHtmlTable.getUndefinedFlag(addParams) &&
						!fnPopUpHtmlTable.getUndefinedFlag(addParams.initPage)){
						initPage = addParams.initPage;
				}

				if(!fnPopUpHtmlTable.getUndefinedFlag(collBackFc)){
					fnPopUpHtmlTable.doTableSearch(initTable, initPage, collBackFc);
				}else{
					fnPopUpHtmlTable.doTableSearch(initTable, initPage);
				}
				}else if(!fnPopUpHtmlTable.getUndefinedFlag(collBackFc)){ // execution call back function
					eval(collBackFc);
				}
			}catch(e){ // exception
				console.log(e);
		...
		tableInitAddRows : function(addCount, collBackFc){ // All table add rows
			for(var i=0; i<fnPopUpHtmlTable.tableNames.length; i++){
				for(var j=0; j<addCount; j++){
					$("#" + fnPopUpHtmlTable.tableNames[i]).append(fnPopUpHtmlTable.tableBodys[i].replace(/_0/g, "_" + fnPopUpHtmlTable.getTableMaxRowNum(i)));
				}
			}
		...
		tableAddRows : function(tableIndex, addCount, collBackFc){ // one table add rows
			...
			for(var i=0; i<count; i++){
				$("#" + fnPopUpHtmlTable.tableNames[tableIndex]).append(fnPopUpHtmlTable.tableBodys[tableIndex].replace(/_0/g, "_" + fnPopUpHtmlTable.getTableMaxRowNum(tableIndex)));
			}
		...
		tableDelRows : function(tableIndex, collBackFc){
			var chkBoxArr = $("#"+fnPopUpHtmlTable.getTableIdByIndex(tableIndex)).find("[id^='chk_']:checked");

			for(var i=0; i<chkBoxArr.length; i++){
				$(chkBoxArr[i]).closest("li").remove();
			}
			if(typeof(collBackFc) != 'undefined'){ // execution call back function
				eval(collBackFc);
			}
		...
		getTableIndexById : function(tableId){
			...
			for(var i=0; i<fnPopUpHtmlTable.tableNames.length; i++){
				if(fnPopUpHtmlTable.tableNames[i] == tableId){
					tIdx = i;
					break;
				}
			}
		...
		getTableIdByIndex : function(tableIndex){
			return fnPopUpHtmlTable.tableNames[tableIndex];
		},
		setTableMultiCombo : function(tObj){
			if($(tObj)[0].tagName != "SELECT"){
				tObj = $(event.target);
			...
			// 변경할 JSONDATA List를 가져오기 위한 Table Index
			var tableIdx = fnPopUpHtmlTable.getTableIndexById($(tObj).closest("table").attr("id"));
			// 상위 콤보 ID
			var parentId = $(tObj).attr("id");
			// 하위 콤보 ID (구분자 , 로 여러개 가능)
			var childArr = $(tObj).attr("child").split(",");
			...
			for(var i=0; i<childArr.length; i++){
				...
				var childId  = $("#" + childArr[i]).attr("id");
				// 변경할 하위콤보 JSONDATA List를 가져온다.
				var multiData = fnPopUpHtmlTable.tableMulti[tableIdx][childId.substring(0, childId.lastIndexOf("_") + 1) + "0"];
		...
		setTableClear : function(tableIndex, collBackFc){ // one table clear
			if(typeof($("#" + fnPopUpHtmlTable.tableNames[tableIndex])) == 'undefined'){
				for(var i=0; i<fnPopUpHtmlTable.tableNames.length; i++){
					if($("#"+fnPopUpHtmlTable.tableNames[i]) == 'undefined'){
						alert("Wrong " + (i + 1) + " Index Table ID");
						return;
					}

					$("#"+fnPopUpHtmlTable.tableNames[i]).empty();
				}
			}else{
				$("#" + fnPopUpHtmlTable.tableNames[tableIndex]).empty();
			}
		...
		getTableMaxRowNum : function(tableIndex){ // one table max rownum
			var trs = fnPopUpHtmlTable.getTableTrsArray(tableIndex);
			var maxRowNum = 0;
			var tmpRowNum = 0;

			for(var i=0; i<trs.length; i++){
				tmpRowNum = fnPopUpHtmlTable.getTableTrNumByTr(trs[i]);
				if( parseInt(maxRowNum, 10) < parseInt(tmpRowNum, 10) ||
					parseInt(maxRowNum, 10) == parseInt(tmpRowNum, 10) ){
					maxRowNum = parseInt(tmpRowNum, 10) + 1;
			}
		...
		getTableLisArray : function(tableIndex){ // get one table all tr
			...
			try{
				trs = $("#" + fnPopUpHtmlTable.tableNames[tableIndex] + ">li");
			}catch(e){
				alert("Except Get TR Array!");
			}
			return trs;
		...
		getTableTrsArray : function(tableIndex){ // get one table all tr
			...
			try{
				trs = $("#" + fnPopUpHtmlTable.tableNames[tableIndex] + ">tr");
			}catch(e){
				alert("Except Get TR Array!");
			}
			return trs;
		...
		getTableChkRow : function(tableIndex){
			var chkBoxArr = $("#" + fnPopUpHtmlTable.getTableIdByIndex(tableIndex)).find("[id^=chk]:checked");
			var idArr = [];
			for(var i=0; i<chkBoxArr.length; i++){
				idArr.push($(chkBoxArr[i]).attr("id").replace("chk_", ""));
			}
	...
	getSearchOption : function(tableIndex, getType){
		var tableName = fnPopUpHtmlTable.tableNames[tableIndex];
		var tabId = "tab";
		if(tableName.indexOf("popUp") == 0){
			tabId = "popUpTab";
		}
		...
		params["hid_cond"]    = optionStr;
		params["hid_pagenum"] = $("#"+ fnPopUpHtmlTable.tableNavis[tableIndex]).find(".active").text();
	...
	doTableSearch : function(tableIndex, nPage, successFc, addParam){
		var url = fnPopUpHtmlTable.tableUrls[tableIndex].search;
//			url = url.substring(0,1) == "/" ? ctxPath+url : url;

var params = fnPopUpHtmlTable.getSearchOption(tableIndex);

for(add in addParam){
	params[add] = addParam[add];
}

if( $("#"+fnPopUpHtmlTable.tableNavis[tableIndex]) != 'undefined' &&
	nPage == 'undefined'){
	nPage = 1;
}

			// common global json data
			params["corpCd"] = sessCorpCd;
			params["locale"]  = sessLocale == "" ? $("#loc").val() : sessLocale;
			...
			$.ajax( {
				...
				,success : function(jsonData) {
					if( $("#"+fnPopUpHtmlTable.tableNavis[tableIndex]) != 'undefined'){
						$("#"+fnPopUpHtmlTable.tableNavis[tableIndex]).html(fnPopUpHtmlTable.getPageNavigator(nPage, jsonData.records, tableIndex));
					}

	                /*if(!fnPopUpHtmlTable.getUndefinedFlag($("#mobile_total")[0])){
	                	$("#mobile_total").text(jsonData.records);
	                }*/

	                $("#"+fnPopUpHtmlTable.tableNames[tableIndex]).empty();
	                if( typeof($(fnPopUpHtmlTable.tableNames[tableIndex])) != 'undefined'){
	                	fnPopUpHtmlTable.setTableRowDatas(
	                		fnPopUpHtmlTable.tableNames[tableIndex]
	                		, fnPopUpHtmlTable.tableBodys[tableIndex]
	                		, jsonData.rows);
	                }

	                var multiComboArr = $("#"+fnPopUpHtmlTable.tableNames[tableIndex]).find(".multi_first");

	                if(multiComboArr.length > 0){
	                	for(var i=0; i<multiComboArr.length; i++){
	                		fnPopUpHtmlTable.setTableMultiCombo(multiComboArr[i]);
	                	}
	                }
		...
		doTableDetail : function(tableIndex, paramData, successFc){
			var tr     = $(event.target).closest("li");
			var rowId  = selectNum($(tr).attr("id"));
			var tblIdx = 0;

			if(tableIndex == ""){
				tblIdx = fnPopUpHtmlTable.getTableIndexById($(event.target).closest("table").attr("id"));
			}else{
				tblIdx = tableIndex;
			}

			var keys = $(tr).find(".key");
			var dtlParam = fnPopUpHtmlTable.getSearchOption(tblIdx, "DTL");

			var paramKey = "";
			var paramVal = "";
			var url      = "";

			if( fnPopUpHtmlTable.getUndefinedFlag($("#url_type_" + rowId)[0]) ){
				url = fnPopUpHtmlTable.tableUrls[tblIdx].detail;
			}else{
				url = fnPopUpHtmlTable.tableUrls[tblIdx]["detail_"+fnPopUpHtmlTable.getColValue("url_type", rowId)];
			}
			...
			fnPopUpHtmlTable.doFormSubmit(url, dtlParam);
		},
		doFormSubmit : function(url, param, Id){
			var formId = "hideForm";
				...
				$.each(datas, function(idx, opt){
					if( $("#"+idx+"_"+(iRow+1))[0] != null){
						switch($("#"+idx+"_"+(iRow+1)).prop("tagName").toLowerCase()){
							...
							case "span" :
							case "p" :
							if($("#"+idx+"_"+(iRow+1)).hasClass("amt")){
								$("#"+idx+"_"+(iRow+1)).html(numberComma(opt));
							}else{
								$("#"+idx+"_"+(iRow+1)).html(opt);
							}
							break;
...
$("#"+idx+"_"+(iRow+1)).hide();
	...
	if(idx.indexOf('file') > -1){
		$("#"+idx+"_"+(iRow+1)).attr("src", ctxPath+"/images/btn/btn_filedown_tbl.png");
	} else {
...
break;
case "input" :
switch($("#"+idx+"_"+(iRow+1)).prop("type").toLowerCase()){
	case "checkbox" :
	$("#"+idx+"_"+(iRow+1)).prop("checked", opt == "Y" ? true : false);
	break;
	default :
	if($("#"+idx+"_"+(iRow+1)).hasClass("amt")){
		$("#"+idx+"_"+(iRow+1)).val(numberComma(opt));
	}else{
		$("#"+idx+"_"+(iRow+1)).val(opt);
	}
}
...
$("#"+idx+"_"+(iRow+1)).val(opt);
		...
		getPageNavigator : function(nowPage, totalCnt, tableIndex){
			...
			if(totalCnt == 0){
	            ...
	            if (start_page_num > 1) {                                                                                       // 현재 페이지가 첫 페이지가 아닐 경우
	                pageNavigator += "<a class=\"first\" href=\"javascript:fnPopUpHtmlTable.pageMove('1', " + tableIndex + ");\"><span class=\"icon\"></span></a>";             // 첫 페이지
	                pageNavigator += "<a class=\"prev\" href=\"javascript:fnPopUpHtmlTable.pageMove('" + (start_page_num - pageNumSize) + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";             // 이전 페이지
	              }
	              for (var i = start_page_num; i <= end_page_num; i++) {
	              	if (nowPage == i) {
	                    pageNavigator += "<a class=\"active\" href=\"javascript:;\">" + i + "</a>";     // 현재 페이지
	                  } else {
	                    pageNavigator += "<a href=\"javascript:fnPopUpHtmlTable.pageMove('" + i + "', " + tableIndex + ");\">" + i + "</a>";           // 다른 페이지
	                  }
	                }
	            if (total_page_cnt > end_page_num) {                                                                            // 마지막 페이지가 전체 페이지보다 작을 경우
	                pageNavigator += "<a class=\"next\" href=\"javascript:fnPopUpHtmlTable.pageMove('" + next_end_page_num + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";              // 다음페이지
	                pageNavigator += "<a class=\"last\" href=\"javascript:fnPopUpHtmlTable.pageMove('" + total_page_cnt + "', " + tableIndex + ");\"><span class=\"icon\"></span></a>";                // 마지막 페이지
	              }
	            }
	          ...
	          pageMove : function(page, tIndex){
	          	var moveCallBack = "";
	          	if(fnPopUpHtmlTable.getUndefinedFlag(fnPopUpHtmlTable.tableUrls[tIndex].search_callback)){
	          		fnPopUpHtmlTable.doTableSearch(tIndex, page);
	          	}else{
	          		fnPopUpHtmlTable.doTableSearch(tIndex, page, fnPopUpHtmlTable.tableUrls[tIndex].search_callback);
	          	}
	          },
	          getUndefinedFlag : function(object){
	          	return typeof(object) == 'undefined';
	          ...
	          setTableReset : function(collBackFc){
	          	for(var i=0; i<fnPopUpHtmlTable.tableNames.length; i++){
	          		fnPopUpHtmlTable.setTableClear(i);
	          		$("#" + fnPopUpHtmlTable.tableNames[i]).append(fnPopUpHtmlTable.tableBodys[i]);
	          	}
	          ...
	          showNewPop : function(targetId, width, height, param){
	          	if(fnPopUpHtmlTable.getUndefinedFlag(param)){
	          		$("#" + targetId).find("input").val("");
	          		var selectBox = $("#" + targetId).find("select");
	          		for(var i=0; i<selectBox.length; i++){
	          			selectBox[i].selectedIndex = "0";
 ...
 */
 ...
 function fnCallLayerPopUp(popUpUrl, width, height, top, left){
 	var _LPopUpBackground = $('<div id="popup_mask" class="popup_mask"></div>');
 	var _LPopUpContent = $('<div id="popup" class="popup_window"></div>');

 	if(width != undefined){
 		_LPopUpContent.css("width",width);
 	}
 	if(height != undefined){
 		_LPopUpContent.css("height",height);
 	}
 	if(top != undefined){
 		_LPopUpContent.css("top",top);
 	}
 	if(left != undefined){
 		_LPopUpContent.css("left",left);
 	}

 	$("body").append(_LPopUpBackground);
 	$("body").append(_LPopUpContent);
 ...
 function fnCallLayerPopUp2(popUpUrl, width, height, top, left){
 	var _LPopUpBackground = $('<div id="popup_mask2" class="popup_mask"></div>');
 	var _LPopUpContent = $('<div id="popup2" class="popup_window"></div>');
 ...
 function fnCallDeptSearchPopUp() {
 	fnCallLayerPopUp("/popup/deptPopup.do");
 }
 function fnCallUserSearchPopUp() {
 	fnCallLayerPopUp("/popup/userPopup.do");
 }
 function doFileAttachPopUp(fileName) {
 	fnCallLayerPopUp("/hncis/popup/"+fileName+"_filePopup.gas",eval(screen.width)/5,eval(screen.height)/4);
 }

 function fnCallDeptSearchPopUp2() {
 	fnCallLayerPopUp2("/popup/deptPopup2.do", "700", "800", "80", "72");
 }

 function fnCallvehicleListPopUp(fileName) {
 	fnCallLayerPopUp("/popup/vehicleListPopUp.do?fileName="+fileName);
//	fnCallLayerPopUp("/hncis/popup/pop_vehicleList"+fileName+".gas");
}

function fnCallvehicleLogListPopUp() {
	fnCallLayerPopUp("/hncis/popup/pop_vehicleLogList.gas");
}

function fnCallStandardListPopUp() {
	fnCallLayerPopUp("/hncis/businessTravel/xbt01_standard.gas");
}

function fnCallXbt01FilePopUp() {
	fnCallLayerPopUp("/hncis/businessTravel/xbt01_file.gas", "1000", "800");
}

function fnCallZipCodePopup() {
	fnCallLayerPopUp("/hncis/popup/zipCodePopup.gas");
}
...
function layer_data_close() {  
	$("#layer_data_close").bPopup({
		modalClose: false,
		opacity: 0.6,
            positionStyle: 'fixed' //'fixed' or 'absolute'
          });
